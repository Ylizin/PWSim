...
from models.BERT.ALBERT import BERT
import numpy as np
...
#                 f1,_ = self.feature_extractor([torch.LongTensor(s) for s in id1],self.get_BoWs(id1),self.vae)
                if self.use_ext_query:
                    ...
                    f1,_ = self.feature_extractor([' '.join([self.di.id2token[i] for i in s])for s in id1])
...
#                 f1 = torch.stack([torch.mean(self.emb(torch.LongTensor(i)),dim = 0) for i in id1],dim=0).cuda()
                f2,_ = self.feature_extractor(text2.tolist())
            ...
            for _ids in self.ids_loader:
                ...
                _f,*_ = self.feature_extractor(_t.tolist())
                _f = torch.mean(_f,dim = 1)
                all_f.append(_f.cpu())
...
#                 text_bow = self.get_BoWs([self.data_set.pos[test_k][0]])
                ...
                test_f,*_ = self.feature_extractor(text1)
                test_f = torch.mean(test_f,dim = 1).cpu()
