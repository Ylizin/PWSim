...
class LSTMEval(GeneralEval):
    def __init__(self,args=_args,train_test_id=0):
        ...
        self.model_str = "topic_bert_asyn"
    ...
    def init_fe(self):
        self.feature_extractor0 = LSTMModule(suffix = str(self.train_test_id))
        self.feature_extractor1= BERTModule(suffix = str(self.train_test_id))

        self.emb = self.feature_extractor0.embedding


    def init_optim(self):
        self.optim = optim.Adam([{'params':itertools.chain(self.feature_extractor0.parameters(),self.proj.parameters())},{'params':self.feature_extractor1.parameters()},{'params':self.vae.parameters()}],lr = self.args.bertLr,weight_decay = self.args.weight_decay)
...
#         self.optim = optim.Adam(self.feature_extractor.parameters(),self.args.lr,weight_decay = self.args.weight_decay)
    ...
    def train(self):
        ...
        for i in range(self.args.nepochs): #s
            epoch_loss = 0
            self.feature_extractor0.train()
            self.feature_extractor1.train()
            self.vae.train()
...
#                 f1,_ = self.feature_extractor([torch.LongTensor(s) for s in id1],self.get_BoWs(id1),self.vae)
                if self.use_ext_query:
                    ...
                    f1,_ = self.feature_extractor0([torch.LongTensor(s) for s in self.query_ext(id1)],self.get_BoWs(self.query_ext(id1)),self.vae)    # the query using ext texts as raw text
                else:
                    # query 向量， 使用原始的tag作为query
                    # f1,_ = self.feature_extractor0([torch.LongTensor(s) for s in id1],self.get_BoWs(self.query_ext(id1)),self.vae)
                    f1 = torch.stack([torch.mean(self.emb(torch.LongTensor(i)),dim = 0) for i in id1],dim=0).cuda()

                   
                f2,_ = self.feature_extractor1(text2.tolist(),bows2.tolist(),self.vae)
            ...
            for _ids in self.ids_loader:
                ...
                _f,*_ = self.feature_extractor1(_t.tolist(),_b,self.vae)

                all_f.append(self.proj(_f).cpu())
...
#                 text_bow = self.get_BoWs([self.data_set.pos[test_k][0]])
                ...
                # test_f,*_ = self.feature_extractor0(text1,text_bow,self.vae)
