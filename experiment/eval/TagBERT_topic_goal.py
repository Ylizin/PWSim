...
from models.BERT.TopicGBERT import BERTModule
import numpy as np
...
class LSTMEval(GeneralEval):
    def __init__(self,args=_args,train_test_id=0):
        ...
        self.model_str = "topic_bert_goal"
...
#                 f1,_ = self.feature_extractor([torch.LongTensor(s) for s in id1],self.get_BoWs(id1),self.vae)
                if self.use_ext_query:
                    ...
                    f1,_ = self.feature_extractor([' '.join([self.di.id2token[i] for i in s])for s in id1],self.get_BoWs(self.query_ext(id1)),ids,self.vae)
...
#                 f1 = torch.stack([torch.mean(self.emb(torch.LongTensor(i)),dim = 0) for i in id1],dim=0).cuda()
            ...
            for _ids in self.ids_loader:
                _t = self.raw[_ids.cpu().numpy()].tolist()
                _g = self.goal_ids[_ids.cpu().numpy()].tolist()
...
#                 text_bow = self.get_BoWs([self.data_set.pos[test_k][0]])
                ...
                ids = [torch.LongTensor(self.data_set.pos[test_k][0])]
                
                pos_ids = list(self.data_set.pos[test_k][1])
                test_f,*_ = self.feature_extractor(text1,text_bow,ids,self.vae)
