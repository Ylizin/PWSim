...
from models.LSTM.TopicAttGoalLSTM import LSTMModule
import numpy as np
...
class LSTMEval(GeneralEval):
    def __init__(self,args=_args,train_test_id=0):
        ...
        self.goal_ids = self.data_set.goal_ids
...
#         self.vae1 = torch.load(self.vae_path+'1')
    ...
    def init_fe(self):
        ...
        self.emb2 = self.feature_extractor.goalEmbedding
        self.sig = nn.Sigmoid()
        self.relu = nn.ReLU()
    ...
    def train(self):
        ...
        for i in range(self.args.nepochs): #s
            ...
            for id1,id2,l in self.train_data_loader:
                ...
                goals = self.goal_ids[id2]
                # 用于生成topic的 bow分布，是用丰富后的还是丰富前的
                bows2 = self.ext_df[id2] 
#                 bows2 = self.bow[id2]
                if self.use_ext_query:
                    # query 向量， 使用ext之后的tag作为query
                    ids = [torch.LongTensor(s) for s in self.query_syn(id1)]
                    f1,_ = self.feature_extractor(ids,self.get_BoWs(self.query_ext(id1)),ids,self.vae)    # the query using ext texts as raw text
                else:
                    ids = [torch.LongTensor(s) for s in id1]
                    # query 向量， 使用原始的tag作为query
                    # f1,_ = self.feature_extractor(ids,self.get_BoWs(self.query_ext(id1)),ids,self.vae)
...
#                 f1 = torch.stack([torch.mean(self.emb(torch.LongTensor(i)),dim = 0) for i in id1],dim=0).cuda()
                f2,_ = self.feature_extractor(text2.tolist(),bows2.tolist(),goals,self.vae)
            ...
            for _ids in self.ids_loader:
                ...
                _g = self.goal_ids[_ids.cpu().numpy()]
#                 _b = self.bow[_ids.cpu().numpy()]
                _b = self.ext_df[_ids.cpu().numpy()]
                _f,*_ = self.feature_extractor(_t,_b,_g,self.vae)
            ...
            for test_k in self.test_keys:
                ...
                # test_f,wei = self.feature_extractor(text1,text_bow,text1,self.vae)
