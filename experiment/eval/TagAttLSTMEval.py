...
from models.Atten.AttenLSTM import AttentionLSTM
import numpy as np
    ...
    def train(self):
        for i in range(self.args.nepochs): #s
            ...
            for id1,id2,l in self.train_data_loader:
                ...
                # and tolist it  
                f1,f2 = self.feature_extractor(text1,text2.tolist())
                if self.cuda:
                    l = l.cuda()
                p = self.cos(f1,f2)
                loss = self.loss(p,l)   
